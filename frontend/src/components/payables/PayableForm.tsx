/**
 * PayableForm - Presentational Component
 * Form for creating and editing payables (accounts payable)
 *
 * Features:
 * - react-hook-form with Zod validation
 * - Fields: contactId, amount, date, dueDate, description, invoiceNumber, notes, branchId
 * - ContactSelector for suppliers
...
import type { CreatePayableInput, UpdatePayableInput, Payable } from '#/entity';
...
/**
 * Zod schema for creating/updating a payable
 * Matches backend CreatePayableDto/UpdatePayableDto validation rules
 */
const payableSchema = z
  .object({
    contactId: z.string().min(1, { message: 'المورد مطلوب' }),
    amount: z.preprocess(
      (val) => {
...
type FormData = z.infer<typeof payableSchema>;
...
export interface PayableFormProps {
  mode: 'create' | 'edit';
  initialData?: Payable;
  onSubmit: (data: CreatePayableInput | UpdatePayableInput) => Promise<void>;
  onCancel?: () => void;
