 ...
 * Form for recording payments on payables (accounts payable)
 *
 * Features:
...
import type { PayPayableDto, AccountPayable } from '@/types/payables.types';
...
/**
 * Zod schema for recording a payment on a payable
 * Matches backend PayPayableDto validation rules
 */
const createPaymentSchema = (maxAmount: number) =>
  z.object({
    amountPaid: z.preprocess(
      ...
      z
        .number({
          required_error: 'المبلغ المدفوع مطلوب',
    ...
    paymentMethod: z.nativeEnum(PaymentMethod, {
      ...
      invalid_type_error: 'طريقة دفع غير صالحة',
    }),
...
export interface PaymentFormProps {
  ...
  onSubmit: (data: PayPayableDto) => Promise<void>;
  onCancel?: () => void;
...
export function PaymentForm({ payable, onSubmit, onCancel, isSubmitting }: PaymentFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    resolver: zodResolver(createPaymentSchema(payable.remainingAmount)),
    defaultValues: {
      amountPaid: payable.remainingAmount, // Default to full remaining amount
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-4">
      {/* Payable Info Display */}
      <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <span className="font-medium text-gray-700">المبلغ الأصلي:</span>
            <span className="mr-2 text-gray-900">{payable.originalAmount.toLocaleString('en-US')} IQD</span>
          </div>
          <div>
            <span className="font-medium text-gray-700">المبلغ المتبقي:</span>
            <span className="mr-2 text-gray-900 font-semibold">
              {payable.remainingAmount.toLocaleString('en-US')} IQD
            </span>
          </div>
          {payable.contact && (
            <div className="col-span-2">
              <span className="font-medium text-gray-700">المورد:</span>
              <span className="mr-2 text-gray-900">{payable.contact.name}</span>
            </div>
          )}
        </div>
      </div>
