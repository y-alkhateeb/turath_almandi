...
import type { Contact, CreateContactInput, UpdateContactInput } from '#/entity';
...
export const useUpdateContact = () => {
  ...
  return useMutation<Contact, ApiError, { id: string; data: UpdateContactInput }>({
...
/**
 * useReactivateContact Hook
 * Mutation to reactivate a deactivated contact
 *
 * @returns Mutation object with mutate/mutateAsync
 *
 * @example
 ...
 *   await reactivateContact.mutateAsync(contactId);
 * };
 * ```
 */
export const useReactivateContact = () => {
  ...
  return useMutation<Contact, ApiError, string>({
    mutationFn: contactService.reactivate,

    onSuccess: (reactivatedContact) => {
      // Invalidate contacts
      queryClient.invalidateQueries({ queryKey: queryKeys.contacts.all });
      queryClient.invalidateQueries({
        queryKey: queryKeys.contacts.detail(reactivatedContact.id),
      });

      // Show success toast
      toast.success('تم إعادة تفعيل جهة الاتصال بنجاح');
    },
  });
};
...
export const useSuppliers = (filters?: Omit<ContactQueryFilters, 'type'>) => {
  return useContacts({ ...filters, type: 'SUPPLIER' });
};
...
export const useCustomers = (filters?: Omit<ContactQueryFilters, 'type'>) => {
  return useContacts({ ...filters, type: 'CUSTOMER' });
};
