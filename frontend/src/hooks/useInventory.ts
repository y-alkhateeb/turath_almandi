  ...
  return useMutation<InventoryItem, ApiError, CreateInventoryInput>({
    ...
    onError: (error, _newItem, context) => {
      ...
      if (error.statusCode === 400) {
        ...
        toast.error('ليس لديك صلاحية لإضافة عناصر المخزون');
  ...
  >({
    ...
    onError: (error, { id }, context) => {
      ...
      if (error.statusCode === 404) {
        ...
        toast.error('ليس لديك صلاحية لتعديل المخزون');
      } else {
  ...
  return useMutation<void, ApiError, string>({
    ...
    onError: (error, _deletedId, context) => {
      ...
      if (error.statusCode === 404) {
        ...
        toast.error('لا يمكن حذف العنصر لأنه مرتبط بسجلات استهلاك');
      } else if (error.statusCode === 403) {
        toast.error('ليس لديك صلاحية لحذف عناصر المخزون');
      } else {
