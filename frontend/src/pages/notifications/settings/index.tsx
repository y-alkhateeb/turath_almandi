...
export default function NotificationSettingsPage() {
  ...
  const handleBack = useCallback(() => {
    if (hasUnsavedChanges) {
      const confirmed = window.confirm('لديك تغييرات غير محفوظة. هل أنت متأكد من المغادرة؟');
      if (!confirmed) return;
    }
    ...
    return (
      <div className="space-y-6">
        ...
        <div dir="rtl">
          ...
          <p className="text-[var(--text-secondary)] mt-1">حدث خطأ أثناء تحميل الإعدادات</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div dir="rtl">
        <h1 className="text-3xl font-bold text-[var(--text-primary)]">إعدادات الإشعارات</h1>
        <p className="text-[var(--text-secondary)] mt-1">قم بتخصيص تفضيلات الإشعارات الخاصة بك</p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4" dir="rtl">
        <p className="text-sm text-blue-800">
          <strong>ملاحظة:</strong> يمكنك تخصيص الإشعارات حسب النوع، وتحديد الفروع التي تريد
          متابعتها، واختيار طريقة العرض (منبثقة، نص، بريد إلكتروني، رسالة نصية).
        </p>
      </div>
      ...
      {updateSettings.isPending && (
        <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50">
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg p-6 shadow-lg flex items-center gap-3">
            ...
            <span className="text-[var(--text-primary)] font-medium">جاري حفظ الإعدادات...</span>
          </div>
        </div>
      )}
    </div>
